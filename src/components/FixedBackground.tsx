...
const FixedBackground: React.FC = () => {
  ...
  useEffect(() => {
    ...
    console.log("FixedBackground: Setting up WebGL");
    ...
    const handleResize = () => {
      ...
      if (canvas && gl) {
        console.log("Resizing canvas to match window");
        canvas.width = window.innerWidth;
        canvas.height = window.innerHeight;
    ...
    const updatePattern = () => {
      ...
      if (!gl || !program) {
        console.warn("Cannot update pattern, WebGL context or program not available");
        // Try again in case this is a temporary issue
    ...
    console.log("Initializing WebGL with delay");
    
    // Set up the initial WebGL context after a slight delay
    const initTimeout = setTimeout(() => {
      if (setupWebGL()) {
        console.log("WebGL setup successful, starting animation loop");
        // Start animation loop immediately to ensure rendering begins
        updatePattern();
      } else {
