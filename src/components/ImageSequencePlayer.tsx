...
const ImageSequencePlayer: React.FC<ImageSequencePlayerProps> = ({
  ...
  const { workingPathFormat } = useImagePathFormat();
  ...
  useEffect(() => {
    ...
    // Only set up scroll sequence for desktop
    if (!isMobile) {
      const resizeSection = () => {
        if (container) {
  ...
  }, [SCROLL_EXTRA_PX, AFTER_VIDEO_EXTRA_HEIGHT, containerRef, isMobile]);

  // Only use scroll sequence on desktop
  if (!isMobile) {
    useScrollSequence({
      containerRef,
    ...
    <div className="fixed top-0 left-0 w-full h-full bg-black flex items-center justify-center">
      <div className="relative w-full h-full">
        {errorMessage ? (
          ...
          <>
            <SequenceImage
              currentFrame={isMobile ? 1 : currentFrame}
