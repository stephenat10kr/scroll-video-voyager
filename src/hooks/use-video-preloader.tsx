...
export const useVideoPreloader = ({
  ...
  const updateProgressWithConstraints = (actualProgress: number) => {
    ...
    const timeBasedProgress = Math.min(100, (elapsedTime / minLoadingTimeMs) * 100);
    ...
    if (elapsedTime > minLoadingTimeMs) {
      ...
      if (elapsedTime > minLoadingTimeMs * 1.1) {
        newProgress = 100;
    ...
    if (elapsedTime > maxLoadingTimeMs && newProgress < 100) {
  ...
  useEffect(() => {
    ...
    }, maxLoadingTimeMs + 2000); // Max time plus buffer
  ...
  }, [videoSrc, loadProgress, minLoadingTimeMs, maxLoadingTimeMs]);
